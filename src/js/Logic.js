export default class Logic {
	...
	handleClick(e) {
		...
		if (target.classList.contains('addTicket') ||
		...
		const statusRound = target.closest('.round:not(.edit-btn):not(.delete-btn)');
	...
	async removeTicket(e) {
		...
		try {
			...
			const popUpContent = this.gui.showVertification(ticket.name, "remove", taskId);
			this.gui.popUp.appendChild(popUpContent);
				...
				newSubmit.addEventListener("click", async (event) => {
					...
					try {
						...
						if (!deleteResponse.ok) {
							...
							throw new Error(`HTTP ошибка ${deleteResponse.status}: ${errorText}`);
						}
	...
	async showDescription(e) {
		...
		try {
			...
			const detailsRow = task.querySelector(`.task-details-row[data-id="details-${taskId}"]`);
			const detailsContent = task.querySelector(`.details-content[data-id="desc-content-${taskId}"]`);
			...
			const allDetails = document.querySelectorAll('.task-details-row:not(.hidden)');
	...
	async addTicket(e) {
		...
		const popUpContent = this.gui.addPopUp("Добавить тикет", "", "", "add", "", "", );
